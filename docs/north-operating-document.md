...
| DEC-22 — DESIGN-01 "quiet ambition" tokens extracted to **`packages/tokens`** (TS-only workspace). Shape unchanged: 4 palettes, 3 type pairings, density helper, easings, Tweaks discriminator. `apps/web/src/app/north/_lib/tokens.ts` becomes a thin re-export so the prototype's existing imports stay green. Native consumers import `@north/tokens` directly | 28 May 2026 | Jordayne Price | Resolves DESIGN-01; web prototype + native UI library now share a single source of truth, no duplication |
| DEC-23 — DESIGN-02 native UI library lands as **`packages/native-ui`**. Base primitives: `Button` (44pt min, primary/outline/ghost), `Card` (surface + optional accent stripe), `Input`, `ProgressBar`, `Icon` (subset of prototype icons used on native). Signal port: `Sparkline` (per-instance gradient IDs via `useId()`), `BandHero`, `RhythmStreakCard`, `WeeklyPulse`, `ConsistencyGrid`, `InputBreakdown`. `react-native-svg@^15.15.5` added as direct dep | 28 May 2026 | Jordayne Price | Resolves DESIGN-02; native app now has a typed component library against the shared tokens. Variants beyond `narrative`/`rhythm`/`grid` land when M2/M3 features need them |
| DEC-24 — DESIGN-03 5-tab nav with raised centre Mission button. Custom `CustomTabBar` supplied via `<Tabs tabBar={...}>` (expo-router keeps routing). 5 routes: `for-you`, `opportunities`, `mission` (centre), `signal`, `profile`. Mission renders as a 50×50 accent circle floated above the bar (`marginTop: -16`); the other 4 stack icon + label vertically. Each button hits `MIN_TOUCH_TARGET` (44pt). Stock `index.tsx` + `two.tsx` deleted | 28 May 2026 | Jordayne Price | Resolves DESIGN-03; native shell matches the prototype's app-shell exactly. Tab screens are placeholders until M1 fills them |
| DEC-25 — DESIGN-04 a11y baseline: WCAG AA contrast check (`scripts/check-token-contrast.ts` asserts ink-on-bg ≥4.5, inkMid-on-bg ≥3, accentInk-on-accent ≥4.5 across all 4 palettes; CI runs `bun run check:contrast`). `MIN_TOUCH_TARGET = 44` exported from `@north/native-ui` and applied to every interactive primitive + tab-bar button. `useReducedMotion()` hook in `@north/native-ui/hooks` wraps `AccessibilityInfo` for component-level animation gating | 28 May 2026 | Jordayne Price | Resolves DESIGN-04; converts the §6.2-implied a11y commitment into mechanical checks so palette tweaks can't accidentally regress contrast and new components inherit the touch-target constant |
