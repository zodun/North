-- Signal score: count monthly-mission step completions in the A component.
--
-- The "A" (follow-through) factor of compute_signal_score counted only
-- public.user_mission_tasks. Active tasks now live in
-- public.monthly_mission_steps, so completions were invisible to the score.
--
-- This redefines compute_signal_score to add the monthly daily steps to A,
-- matched on due_date (their created_at is all month-start, which would fall
-- outside the rolling window). A daily step counts as completed when it is
-- done OR its covering weekly milestone is done, so daily- and weekly-cadence
-- users both get credit. Everything else (C, K, V, guards, payload) is
-- unchanged from 0005. The weekly join is 1:1 per (mission, week_index), so
-- it does not multiply the daily-step count.
--
-- Note: the V (avoidance) factor still reads only user_mission_tasks, since
-- monthly_mission_steps has no abandon tracking — it stays 0 for users on the
-- new system, i.e. no avoidance dampener until that signal is added.
...
declare
    ...
    m_assigned     int;     -- monthly daily steps in window
    m_completed    int;     -- monthly daily steps done (self or via weekly)
    a_window_days  int := 7;
...
begin
    ...
    -- Sums legacy daily-mission tasks (by created_at) and current monthly
    -- daily steps (by due_date; done counts self OR covering weekly milestone).
    ...
    select count(*), count(*) filter (where s.done or w.done)
    into m_assigned, m_completed
    from public.monthly_mission_steps s
    left join public.monthly_mission_steps w
      on w.monthly_mission_id = s.monthly_mission_id
     and w.cadence = 'weekly'
     and w.week_index = s.week_index
    where s.user_id = uid
      and s.cadence = 'daily'
      and s.due_date between target_date - 6 and target_date;

    assigned_a  := assigned_a + coalesce(m_assigned, 0);
    completed_a := completed_a + coalesce(m_completed, 0);
    ...
    if assigned_a < 7 then
        ...
        where s.user_id = uid
          and s.cadence = 'daily'
          and s.due_date between target_date - 13 and target_date;
